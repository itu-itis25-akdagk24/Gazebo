...
drone_0 = Drone("udp:127.0.0.1:14550",0)
drone_1 = Drone("udp:127.0.0.1:14560",1)
drone_2 = Drone("udp:127.0.0.1:14570",2)
...
for drone in drones:	
	...
	print(f"heartbeat {id} received")
	id += 1
while True:
	command=int(input("Enter command. ARM:1 TAKEOFF:2 SWARM MOVE:3 V FORMATION:4 SWARM LAND:5 INVIDUAL CONTROL:6\n"))
	if command==1:
		for drone in drones:
			drone.arm()
	...
	if command==6:
		id =int(input("select drone: 0 1 2: "))
		for drone in drones:
			if drone.id == id:
				command=int(input("Enter command. ARM:1 TAKEOFF:2 MOVE:3 LAND:4\n"))
				if command==1:
					drone.arm()
					print(f"drone armed {drone.id}")
				if command==2:
